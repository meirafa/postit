...
export class NoteService {
  ...
  public async getOne(
    ...
    if (!note) throw new NotFoundException('A nota não foi encontrada');

    if (note.isPublic) return note;

    if (note.userId === requestUser.id) return note;

    throw new ForbiddenException(
      'Você não tem permissão para visualizar essa nota',
    );
  }
  ...
  public async updateOne(
    ...
    if (note.userId !== requestUser.id)
      throw new ForbiddenException(
        'Você não tem permissão para atualizar essa nota',
      );
